...
local function _WarnQuestNameLookupMissOnce(questStarter, unitType, questgiverID, name)
    local missKey = tostring(unitType) .. ":" .. tostring(questgiverID) .. ":" .. tostring(questStarter) .. ":" .. tostring(name)
    if _questNameLookupMisses[missKey] then
        return
    end

    _questNameLookupMisses[missKey] = true
    Questie:Debug(
        Questie.DEBUG_INFO,
        "[QuestieDB.GetQuestIDFromName] No DB mapping for quest frame entry. Using fallback icon.",
        "type:", unitType,
        "id:", questgiverID,
...
local function _FindTrackedQuestIdByName(name)
    if not name then
        return 0
    end

    for questId in pairs(QuestiePlayer.currentQuestlog) do
        if name == QuestieDB.QueryQuestSingle(questId, "name") then
            return questId
        end
    end
...
local function _FindUniqueDoableQuestIdByName(name)
    if not name then
        return 0
    end
    ...
    for questId in pairs(QuestieDB.QuestPointers or QuestieDB.questData) do
        if (name == QuestieDB.QueryQuestSingle(questId, "name")) and QuestieDB.IsDoable(questId) then
            matchedId = questId
            matches = matches + 1
...
function QuestieDB.GetQuestIDFromName(name, questgiverGUID, questStarter)
    ...
    if questgiverGUID then
        ...
        if questStarter == true then
            ...
            if questID == 0 then
                -- CoA/Ascension custom questgivers can reuse quest names without stock questStart links.
                -- If there is a single unique doable quest by name, use it; otherwise fall back to generic icons.
                questID = _FindUniqueDoableQuestIdByName(name)
                if questID == 0 then
                    _WarnQuestNameLookupMissOnce(true, unit_type, questgiverID, name)
                end
            end
        else
            if questsEnded then
            ...
            if questID == 0 then
                -- Active quest lists are safest to resolve from the player's own quest log when custom questEnd links are absent.
                questID = _FindTrackedQuestIdByName(name)
                if questID == 0 then
                    _WarnQuestNameLookupMissOnce(false, unit_type, questgiverID, name)
                end
            end
        end
    end
