...
local startupBuildBannerShown = false

local function _PrintBuildMarker()
    if startupBuildBannerShown or not DEFAULT_CHAT_FRAME then
        return
    end

    startupBuildBannerShown = true
    DEFAULT_CHAT_FRAME:AddMessage("|cff33ffcc[QuestieCustom]|r Loaded Ascension build " .. tostring(QuestieLib:GetAddonVersionString()) .. " |cffaaaaaa(/q335)|r")
end

local function _PrintBuildStatus()
    ...
    if QuestLogCache and QuestLogCache.questLog_DO_NOT_MODIFY then
        for _ in pairs(QuestLogCache.questLog_DO_NOT_MODIFY) do
            cacheCount = cacheCount + 1
        end
    end

    Questie:Print(
        "QuestieCustom Ascension build " .. tostring(QuestieLib:GetAddonVersionString())
        .. ", Client " .. tostring(GetBuildInfo())
        .. ", TrackerEnabled=" .. tostring(Questie.db and Questie.db.profile and Questie.db.profile.trackerEnabled)
        .. ", AutoTrack=" .. tostring(Questie.db and Questie.db.profile and Questie.db.profile.autoTrackQuests)
        .. ", Header=" .. tostring(Questie.db and Questie.db.profile and Questie.db.profile.trackerHeaderEnabled)
        .. ", Started=" .. tostring(QuestieTracker and QuestieTracker.started)
        .. ", Expanded=" .. tostring(Questie.db and Questie.db.char and Questie.db.char.isTrackerExpanded)
        .. ", QuestLog=" .. tostring(questLogCount)
        .. ", Cache=" .. tostring(cacheCount)
    )
end
...
function Questie:OnInitialize()
    ...
    if Questie.db.profile.trackerFontOutline == "Outline" then
        Questie.db.profile.trackerFontOutline = "OUTLINE"
    elseif Questie.db.profile.trackerFontOutline == "Monochrome" then
        Questie.db.profile.trackerFontOutline = "MONOCHROME"
    elseif Questie.db.profile.trackerFontOutline == "None" then
    ...
    -- Early debug marker so this exact build can be verified even if later init stages stall.
    Questie:RegisterChatCommand("q335", _PrintBuildStatus)
    Questie:RegisterChatCommand("questiever", _PrintBuildStatus)
    Questie:RegisterChatCommand("questiebuild", _PrintBuildStatus)
...
Questie.icons = {
    ...
    ["arrow"] = QuestieLib.AddonPath.."Icons\\arrow.tga",
    ...
    ["tracker_achievements"] = QuestieLib.AddonPath.."Icons\\tracker_achievements.tga",
...
Questie.ICON_TYPE_SLAY_MONO = 18
Questie.ICON_TYPE_LOOT_MONO = 19
Questie.ICON_TYPE_OBJECT_MONO = 20
...
function Questie:SetIcons()
    ...
    Questie.usedIcons[Questie.ICON_TYPE_SLAY_MONO] = Questie.icons["slay_mono"]
    Questie.usedIcons[Questie.ICON_TYPE_LOOT_MONO] = Questie.icons["loot_mono"]
    Questie.usedIcons[Questie.ICON_TYPE_OBJECT_MONO] = Questie.icons["object_mono"]
