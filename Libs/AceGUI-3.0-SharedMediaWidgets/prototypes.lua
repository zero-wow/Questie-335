...
do
	...
	local function UpdateContentWidths(self)
		local scrollWidth = self.scrollframe and self.scrollframe:GetWidth() or 0
		local frameWidth = self:GetWidth() or 0
		local fallbackWidth = frameWidth - (self.slider and self.slider:IsShown() and 42 or 28)
		local contentWidth = math.max(0, scrollWidth or 0, fallbackWidth or 0)
		self.contentframe:SetWidth(contentWidth)

		for index, child in ipairs(self.contentRepo) do
			child:ClearAllPoints()
			child:SetWidth(contentWidth)
			if child.UpdateWidth then
				child:UpdateWidth(contentWidth)
			end

			if index == 1 then
				child:SetPoint("TOPLEFT", self.contentframe, "TOPLEFT", 0, 0)
			else
				child:SetPoint("TOPLEFT", self.contentRepo[index - 1], "BOTTOMLEFT", 0, 0)
			end

			child:SetPoint("RIGHT", self.contentframe, "RIGHT", 0, 0)
		end
	end
	...
	local function slider_OnValueChanged(self, value)
		...
		self.frame:UpdateContentWidths()
	end
		...
		if next(DropDownCache) then
			...
			local scrollframe = CreateFrame("ScrollFrame", nil, frame)
				...
				scrollframe:SetScript("OnSizeChanged", function(self)
					if self.frame and self.frame.UpdateContentWidths then
						self.frame:UpdateContentWidths()
					end
				end)
			...
			frame.UpdateContentWidths = UpdateContentWidths
	...
	function AGSMW:ReturnDropDownFrame(frame)
		...
		if frame.searchBox then
			...
			frame.searchBox.ownerWidget = nil
		end
		frame.scrollframe:ClearAllPoints()
		frame.scrollframe:SetPoint("TOPLEFT", frame, "TOPLEFT", 14, -13)
		frame.scrollframe:SetPoint("BOTTOMRIGHT", frame, "BOTTOMRIGHT", -14, 12)
