...
--- Return bearing and distance between two world positions (Astrolabe/TomTom convention).
-- @return angle, distance where angle is in radians compatible with GetPlayerFacing()
...
local function UpdateMinimapZoom()
    if not MinimapRadiusAPI then
        local zoom = tonumber(pins.Minimap:GetZoom()) or 0
        local insideZoom = tonumber(GetCVar("minimapInsideZoom")) or zoom
        local outsideZoom = tonumber(GetCVar("minimapZoom")) or zoom

        if outsideZoom == insideZoom then
            pins.Minimap:SetZoom(zoom < 2 and zoom + 1 or zoom - 1)
        end

        indoors = outsideZoom == (tonumber(pins.Minimap:GetZoom()) or 0) and "outdoor" or "indoor"
...
local function RefreshWorldMapDimensions()
    if not WorldMapButton then
        return false
    end

    -- SetPoint offsets use WorldMapButton's own coordinate space. Applying its
    -- scale again shifts pins when Mapster switches between map layouts.
...
local function HandleWorldMapPin(icon, data)
    ...
    if not RefreshWorldMapDimensions() then
        icon:Hide()
        return
    end
    ...
    --Questie Modification
    if (Questie.db.profile.hideIconsOnContinents == true) and (currentMapData.mapType == Enum.UIMapType.Continent or uiMapID == 947) or (uiMapID ~= data.uiMapID and data.worldMapShowFlag == HBD_PINS_WORLDMAP_SHOW_CURRENT) then
        icon:Hide();
        return;
    ...
    if uiMapID == WORLD_MAP_ID then
        ...
        if uiMapID ~= data.uiMapID then
            ...
            if not data.uiMapID then
                ...
                local pinMapData = HBD.mapData[data.uiMapID]
                if not pinMapData then
                    return
                end
                local parentMapID = pinMapData.parent
...
local function UpdateWorldMap()
    if not WorldMapFrame:IsVisible() then return end
    if not RefreshWorldMapDimensions() then return end
...
local worldMapUpdateQueued = false
local function RequestWorldMapUpdate()
    if worldMapUpdateQueued then
        return
    end

    worldMapUpdateQueued = true
    C_Timer.After(0, function()
        worldMapUpdateQueued = false
        UpdateWorldMap()
    end)
end
pins.RequestWorldMapUpdate = RequestWorldMapUpdate
...
pins.updateFrame:RegisterEvent("WORLD_MAP_NAME_UPDATE")
...
-- WORLD_MAP_UPDATE can fire before the frame becomes visible on 3.3.5. The
-- deferred pass runs after Blizzard and map-addon OnShow/layout handlers.
if WorldMapFrame and WorldMapFrame.HookScript then
    WorldMapFrame:HookScript("OnShow", RequestWorldMapUpdate)
end

if WorldMapButton and WorldMapButton.HookScript then
    WorldMapButton:HookScript("OnSizeChanged", RequestWorldMapUpdate)
end

-- Showing quest objectives does not reliably trigger WORLD_MAP_UPDATE.
