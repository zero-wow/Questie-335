...
function QuestieCompat.GetQuestNameById(questId)
    if not questId then
        return nil
    end
    ...
    if QuestieCompat.abandonQuestID == questId and QuestieCompat.abandonQuestName and QuestieCompat.abandonQuestName ~= "" then
        return QuestieCompat.abandonQuestName
    end

    return nil
end

function QuestieCompat.GetQuestLevelByID(questId)
    if not questId then
        return nil
    end
...
function QuestieCompat:GetQuestLinkString(questLevel, questName, questId)
    questName = questName or QuestieCompat.GetQuestNameById(questId) or ("Quest " .. tostring(questId or "?"))
    questLevel = questLevel or QuestieCompat.GetQuestLevelByID(questId) or "?"
	return QuestieCompat.GetQuestLink(questId) or "[["..tostring(questLevel).."] "..tostring(questName).." ("..tostring(questId)..")]"
end

function QuestieCompat:GetQuestLinkStringById(questId)
    local questName = QuestieCompat.GetQuestNameById(questId)
    local questLevel = QuestieCompat.GetQuestLevelByID(questId)
...
function QuestieCompat.UnitClass(unit)
    ...
    local classId = classFile and QuestieCompat.ChrClasses[classFile]

    -- Ascension can expose custom classes while quest restrictions still use the
    -- underlying WoW base-class bit flags. Use UnitClassBase when available to
    -- recover a stable class id on 3.3.5 clients.
    if not classId and type(UnitClassBase) == "function" then
        local baseClassName, baseClassFile = UnitClassBase(unit)
        classId = baseClassFile and QuestieCompat.ChrClasses[baseClassFile]

        if (not classFile or classFile == "") and baseClassFile then
            classFile = baseClassFile
        end

        if (not className or className == "") and baseClassName then
...
local function SyncFontStringWrapSettings(source, target)
    ...
    if target.SetWordWrap then
        local wordWrap = true
        if source.GetWordWrap then
            wordWrap = source:GetWordWrap()
        end
        target:SetWordWrap(wordWrap ~= false)
    end

    if target.SetNonSpaceWrap then
        local nonSpaceWrap = false
        if source.GetNonSpaceWrap then
            nonSpaceWrap = source:GetNonSpaceWrap()
        end
        target:SetNonSpaceWrap(nonSpaceWrap == true)
    end
end

function QuestieCompat.GetWrappedStringHeight(self)
    local fontName, fontHeight, fontFlags = self:GetFont()
    if not fontHeight or fontHeight <= 0 then
        return 1
    end

    unboundedFS:SetWidth(math.max(1, self:GetWidth() or 1))
    unboundedFS:SetFont(fontName, fontHeight, fontFlags)
    SyncFontStringWrapSettings(self, unboundedFS)
    unboundedFS:SetText(self:GetText() or "")

    return math.max(fontHeight, unboundedFS:GetHeight())
end
...
function QuestieCompat.QuestEventHandler_RegisterEvents()
    ...
    hooksecurefunc("SetAbandonQuest", function()
        local questTitle, _, _, _, _, _, _, _, questId = GetQuestLogTitle(GetQuestLogSelection())
        QuestieCompat.abandonQuestID = questId
        QuestieCompat.abandonQuestName = questTitle
    end)
...
function QuestieCompat.QuestieTracker_Initialize(trackerQuestFrame)
    ...
    if Questie_HeaderFrame and Questie_HeaderFrame.trackedQuests and Questie_HeaderFrame.trackedQuests.label then
