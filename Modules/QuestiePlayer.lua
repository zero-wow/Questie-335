...
function QuestiePlayer:Initialize()
    ...
    if classId then
        ...
        -- CoA / custom-class environments may not expose a stable base class id.
        -- In that case, do not filter quests by class bits.
        playerClassFlag = nil
        playerClassFlagX2 = nil
    end
end
...
function QuestiePlayer.HasRequiredClass(requiredClasses)
    -- test a bit flag: (value % (2*flag) >= flag)
    return (not requiredClasses) or (requiredClasses == 0) or (not playerClassFlag) or ((requiredClasses % playerClassFlagX2) >= playerClassFlag)
end
