...
-- Single-spawn mono pins only (see ApplySingleSpawnIconOverrides).
local SINGLE_SPAWN_MONO_IDLE_GREY = { 0.62, 0.62, 0.62 }

local function _IsSingleSpawnMonoIcon(data)
    return data and data.isSingleSpawnMono
end

local function _GetObjectiveIconColors(data, objectiveColor)
    if _IsSingleSpawnMonoIcon(data) then
        return SINGLE_SPAWN_MONO_IDLE_GREY
    end
    if data.IconColor ~= nil and objectiveColor then
...
function _Qframe:UpdateTexture(texture)
    ...
    local colors = _GetObjectiveIconColors(self.data, objectiveColor)
