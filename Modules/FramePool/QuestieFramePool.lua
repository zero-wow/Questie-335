...
QuestieFramePool.mapShiftClickThrough = false

local function _ApplyWorldMapShiftClickThrough(icon, enabled)
    if not icon or icon.miniMapIcon then
        return
    end

    if enabled then
        if icon._shiftMouseEnabled == nil then
            icon._shiftMouseEnabled = icon:IsMouseEnabled()
        end
        icon:EnableMouse(false)
        if icon.data and icon.data.lineFrames then
            for _, lineFrame in pairs(icon.data.lineFrames) do
                lineFrame:EnableMouse(false)
            end
        end
    else
        icon:EnableMouse(icon._shiftMouseEnabled ~= false)
        icon._shiftMouseEnabled = nil
        if icon.data and icon.data.lineFrames then
            for _, lineFrame in pairs(icon.data.lineFrames) do
...
function QuestieFramePool:SetWorldMapShiftClickThrough(enabled)
    enabled = enabled and true or false
    if self.mapShiftClickThrough == enabled then
        return
    end
    self.mapShiftClickThrough = enabled

    for _, icon in pairs(usedFrames) do
        _ApplyWorldMapShiftClickThrough(icon, enabled)
    end
end

function QuestieFramePool:ApplyWorldMapShiftClickThroughToIcon(icon)
    if self.mapShiftClickThrough then
        _ApplyWorldMapShiftClickThrough(icon, true)
    end
end

function QuestieFramePool:RegisterShiftClickThroughHooks()
    -- Must use Blizzard's frame; QuestieFramePool's local WorldMapFrame is QuestieCompat's wrapper.
    local mapFrame = _G.WorldMapFrame
    if self._shiftHooksRegistered or not mapFrame or not mapFrame.GetScript then
        return
    end

    local function wrap(scriptName, handler)
        if mapFrame.HookScript then
            mapFrame:HookScript(scriptName, handler)
        else
            local orig = mapFrame:GetScript(scriptName)
            mapFrame:SetScript(scriptName, function(frame, ...)
                if orig then
                    orig(frame, ...)
                end
                handler(frame, ...)
    ...
    wrap("OnShow", function()
        if IsShiftKeyDown() then
            QuestieFramePool:SetWorldMapShiftClickThrough(true)
        end
    end)
    wrap("OnHide", function()
        QuestieFramePool:SetWorldMapShiftClickThrough(false)
    end)

    self._shiftHooksRegistered = true
end
...
local SINGLE_SPAWN_MONO_IDLE_GREY = { 0.62, 0.62, 0.62 }
    ...
    if enable then
        for _, icon in pairs(usedFrames) do
            if (mini and icon.miniMapIcon) or (not mini and not icon.miniMapIcon) then
                local colors = {1, 1, 1}
                if icon.data and icon.data.isSingleSpawnMono then
                    colors = SINGLE_SPAWN_MONO_IDLE_GREY
                elseif icon.data and icon.data.IconColor ~= nil then
        ...
        for _, icon in pairs(usedFrames) do
            if (mini and icon.miniMapIcon) or (not mini and not icon.miniMapIcon) then
                if icon.data and icon.data.isSingleSpawnMono then
                    icon.texture:SetVertexColor(SINGLE_SPAWN_MONO_IDLE_GREY[1], SINGLE_SPAWN_MONO_IDLE_GREY[2], SINGLE_SPAWN_MONO_IDLE_GREY[3], 1)
                else
