...
local function _UpdateFallbackObjectiveFromLog(objective, objectiveData)
    local collected = tonumber(objectiveData.numFulfilled) or 0
    local needed = tonumber(objectiveData.numRequired) or 0

    objective.Type = objectiveData.type
    objective.Description = objectiveData.text or objective.Description or ""
    objective.Collected = collected
    objective.Needed = needed
    objective.Completed = objectiveData.finished == true or (needed > 0 and collected >= needed)
end

function _QuestieQuest.FallbackObjectiveUpdate(self)
    local questLogData = QuestLogCache.GetQuest(self.questId)
    if not questLogData or not questLogData.objectives then
        return
    end

    local objectiveData = questLogData.objectives[self.Index]
    if objectiveData then
        _UpdateFallbackObjectiveFromLog(self, objectiveData)
    end
end

local function _BuildLiveQuestFallback(questId, questLogData, includeObjectives, existingQuest)
    local questLogIndex = GetQuestLogIndexByID and GetQuestLogIndexByID(questId)
    local title, level
    if questLogIndex then
        title, level = GetQuestLogTitle(questLogIndex)
    end

    title = (questLogData and questLogData.title) or title
    if not title then
        return nil
    end

    local quest = existingQuest
    if (not quest) or (not quest._liveQuestFallback) then
        quest = {
            Id = questId,
            _liveQuestFallback = true,
            Objectives = {},
            SpecialObjectives = {},
            ObjectiveData = {},
            Description = {},
        ...
        quest.IsComplete = function(self)
            if self._questLogComplete == -1 then
                return -1
            elseif self._questLogComplete == 1 or self.isComplete == true then
                return 1
            end
            return 0
        end
    ...
    quest.level = tonumber(level) or quest.level or 0
    quest.zoneOrSort = quest.zoneOrSort or 0
    quest._questLogComplete = (questLogData and questLogData.isComplete) or 0
    quest.isComplete = quest._questLogComplete == 1 and true or nil
    quest.WasComplete = quest.isComplete == true and quest.WasComplete or nil
    quest.Description[1] = title
    quest.SpecialObjectives = quest.SpecialObjectives or {}
    quest.ObjectiveData = quest.ObjectiveData or {}
    quest.sourceItemId = quest.sourceItemId or 0
    quest.requiredSourceItems = nil
    quest.SpellItemId = nil

    if includeObjectives == false then
        quest.Objectives = {}
        return quest
    end

    local objectives = (questLogData and questLogData.objectives) or {}
    quest.Objectives = quest.Objectives or {}

    for objectiveIndex, objectiveData in ipairs(objectives) do
        ...
        if not objective then
            objective = {
                ...
                spawnList = {},
                AlreadySpawned = {},
                Update = _QuestieQuest.FallbackObjectiveUpdate,
            }
            quest.Objectives[objectiveIndex] = objective
        end

        objective.Id = objective.Id or objectiveIndex
        objective.Index = objectiveIndex
        objective.questId = questId
        objective.spawnList = objective.spawnList or {}
        objective.AlreadySpawned = objective.AlreadySpawned or {}
        objective.Update = _QuestieQuest.FallbackObjectiveUpdate
        _UpdateFallbackObjectiveFromLog(objective, objectiveData)
    end

    for objectiveIndex = #objectives + 1, #quest.Objectives do
        quest.Objectives[objectiveIndex] = nil
    end

    return quest
end

local function _GetQuestFromDatabaseOrFallback(questId, questLogData, includeObjectives)
    local quest = QuestieDB.GetQuest(questId)
    if quest then
        if questLogData and questLogData.title then
            quest.LocalizedName = questLogData.title
        end
        return quest
    end

    return _BuildLiveQuestFallback(questId, questLogData, includeObjectives, QuestiePlayer.currentQuestlog[questId])
end
...
function QuestieQuest:SmoothReset()
    ...
    local stepTable = {
        ...
        function()
            ...
            QuestieQuest._resetAvailablesStarted = GetTime()
            local ok, err = pcall(function()
                AvailableQuests.CalculateAndDrawAll(function() QuestieQuest._resetNeedsAvailables = false end)
            end)
            if not ok then
                Questie:Error("[QuestieQuest:SmoothReset] CalculateAndDrawAll failed:", err)
                QuestieQuest._resetNeedsAvailables = false
            end
            return true
        end,
        ...
        function()
            -- Safety valve: never block forever if available-quest callback was lost
            if QuestieQuest._resetNeedsAvailables and (QuestieQuest._resetAvailablesStarted or 0) > 0 then
                if (GetTime() - QuestieQuest._resetAvailablesStarted) > 15 then
                    Questie:Error("[QuestieQuest:SmoothReset] Available quest redraw timed out; continuing.")
                    QuestieQuest._resetNeedsAvailables = false
                end
            end
...
function QuestieQuest:AcceptQuest(questId)
    local quest = _GetQuestFromDatabaseOrFallback(questId, QuestLogCache.questLog_DO_NOT_MODIFY[questId], true)
...
function QuestieQuest:GetAllQuestIds()
    ...
    for questId, data in pairs(QuestLogCache.questLog_DO_NOT_MODIFY) do -- DO NOT MODIFY THE RETURNED TABLE
        local quest = _GetQuestFromDatabaseOrFallback(questId, data, true)
        if quest then
            local complete = quest:IsComplete()
...
function QuestieQuest:GetAllQuestIdsNoObjectives()
    ...
    for questId, data in pairs(QuestLogCache.questLog_DO_NOT_MODIFY) do -- DO NOT MODIFY THE RETURNED TABLE
        local quest = _GetQuestFromDatabaseOrFallback(questId, data, false)
        if quest then
            QuestiePlayer.currentQuestlog[questId] = quest
            if not quest._liveQuestFallback then
...
_DetermineIconsToDraw = function(quest, objective, objectiveIndex, objectiveCenter)
    ...
    for id, spawnData in pairs(objective.spawnList) do
        ...
        if (not objective.AlreadySpawned[id]) and (not objective.Completed) and Questie.db.profile.enableObjectives then
            ...
            _QuestieQuest.ApplySingleSpawnIconOverrides(spawnData, data)
...
function QuestieQuest:PopulateQuestLogInfo(quest)
    ...
    if quest._liveQuestFallback then
        quest = _BuildLiveQuestFallback(quest.Id, QuestLogCache.questLog_DO_NOT_MODIFY[quest.Id], true, quest)
        if quest then
            QuestiePlayer.currentQuestlog[quest.Id] = quest
            return true
        end
        return nil
    end
