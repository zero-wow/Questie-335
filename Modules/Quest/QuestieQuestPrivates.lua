...
local SINGLE_SPAWN_MONO_SCALE = 1.5

---@param spawnData SpawnListBase
---@return number
local function _CountSpawnCoords(spawnData)
    local count = 0
    if spawnData and spawnData.Spawns then
        for _, spawns in pairs(spawnData.Spawns) do
...
--- Unique map pins (one coordinate for a spawn source) use mono objective icons.
---@param spawnData SpawnListBase
---@param data table
function _QuestieQuest.ApplySingleSpawnIconOverrides(spawnData, data)
    if _CountSpawnCoords(spawnData) ~= 1 then
        return
    end

    local monoIcon
    if spawnData.Icon == Questie.ICON_TYPE_SLAY then
        monoIcon = Questie.ICON_TYPE_SLAY_MONO
    elseif spawnData.Icon == Questie.ICON_TYPE_LOOT then
        monoIcon = Questie.ICON_TYPE_LOOT_MONO
    elseif spawnData.Icon == Questie.ICON_TYPE_OBJECT then
        monoIcon = Questie.ICON_TYPE_OBJECT_MONO
    elseif spawnData.Icon == Questie.ICON_TYPE_EVENT then
        -- Zone trigger objectives (triggerEnd) are coordinate-only; show interact cog like single objects.
        monoIcon = Questie.ICON_TYPE_OBJECT_MONO
    end

    if not monoIcon then
        return
    end

    data.Icon = monoIcon
    data.isSingleSpawnMono = true
    data.GetIconScale = function()
        return SINGLE_SPAWN_MONO_SCALE
    end
    data.IconScale = SINGLE_SPAWN_MONO_SCALE
end
