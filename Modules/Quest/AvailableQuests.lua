...
function AvailableQuests.CalculateAndDrawAll(callback)
    ...
    -- Canceling the previous timer must still complete its callback (SmoothReset depends on it)
    if timer then
        timer:Cancel()
        timer = nil
        if AvailableQuests._pendingDrawCallback then
            local pending = AvailableQuests._pendingDrawCallback
            AvailableQuests._pendingDrawCallback = nil
            pcall(pending)
        end
    end

    AvailableQuests._pendingDrawCallback = callback
...
function AvailableQuests.UnloadUndoable()
    for questId, _ in pairs(availableQuests) do
        if (not QuestieDB.IsDoable(questId)) then
            ...
            if (not QuestiePlayer.currentQuestlog[questId]) and (not QuestLogCache.questLog_DO_NOT_MODIFY[questId]) then
...
_CalculateAvailableQuests = function()
    ...
    local function _DrawQuestIfAvailable(questId)
        ...
        if currentQuestlog[questId] then
            -- Stale available-flag must not allow UnloadQuestFrames on active quest notes later
            availableQuests[questId] = nil
        ...
        if (
            ...
            if availableQuests[questId] then
                availableQuests[questId] = nil
                -- Never unload frames for quests currently in the log (objectives / turn-ins)
                if (not currentQuestlog[questId]) and (not QuestLogCache.questLog_DO_NOT_MODIFY[questId]) then
                    QuestieMap:UnloadQuestFrames(questId)
