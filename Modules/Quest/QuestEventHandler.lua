...
local trackerZoneRefreshScheduled = false
local function _ScheduleTrackerZoneRefresh()
    if trackerZoneRefreshScheduled
        or not Questie.db.profile.collapseCompletedQuests
        or not Questie.db.profile.collapseCompletedQuestsCurrentZoneOnly
    then
        return
    end

    trackerZoneRefreshScheduled = true
    C_Timer.After(0.2, function()
        trackerZoneRefreshScheduled = false
        QuestieCombatQueue:Queue(function()
            QuestieTracker:Update()
        end)
    end)
...
function _QuestEventHandler:OnEvent(event, ...)
    ...
    elseif event == "ZONE_CHANGED" or event == "ZONE_CHANGED_INDOORS" then
        _ScheduleTrackerZoneRefresh()
    elseif string.sub(event, 1, 4) == "LFG_" then
        QuestieTracker:RequestLFGObjectiveMirrorRefresh()
