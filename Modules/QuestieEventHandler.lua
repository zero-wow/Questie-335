...
function QuestieEventHandler:RegisterLateEvents()
    QuestieFramePool:RegisterShiftClickThroughHooks()
    ...
    Questie:RegisterEvent("MERCHANT_SHOW", QuestieAuto.MERCHANT_SHOW)
    Questie:RegisterEvent("MERCHANT_UPDATE", QuestieAuto.MERCHANT_UPDATE)
    Questie:RegisterEvent("MERCHANT_CLOSED", QuestieAuto.MERCHANT_CLOSED)
...
function _EventHandler:ModifierStateChanged(key, down)
    if key == "LSHIFT" or key == "RSHIFT" then
        local shiftDown = down == 1
        local worldMapOpen = WorldMapFrame and WorldMapFrame:IsShown()
        local onWorldMap = worldMapOpen and MouseIsOver(WorldMapFrame)

        if worldMapOpen or QuestieFramePool.mapShiftClickThrough then
            QuestieFramePool:SetWorldMapShiftClickThrough(shiftDown and worldMapOpen)
        end

        if onWorldMap and shiftDown then
            local GameTooltip = QuestieCompat.Tooltip
            if GameTooltip and GameTooltip:IsShown() and GameTooltip.ShownAsMapIcon then
                GameTooltip:Hide()
        ...
        elseif onWorldMap or MouseIsOver(Minimap) then
            ...
            -- Questie Map Icons (rebuild for shift-expanded tooltip content; skip while click-through active)
...
function _EventHandler:PlayerRegenEnabled()
    ...
    QuestieTracker:SetLFGObjectiveCombatFallback(false)
