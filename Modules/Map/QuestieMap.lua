...
function QuestieMap.GetScaleValue()
    ...
    if C_Map and C_Map.GetMapInfo then
        ...
        if mapInfo and (mapInfo.mapType == 0) then     --? Cosmic, This is probably not needed but for the sake of completion...
            scaling = 0.85
        elseif mapInfo and (mapInfo.mapType == 1) then -- World
            scaling = 0.85
        elseif mapInfo and (mapInfo.mapType == 2) then -- Continent
            scaling = 0.9
        end
    end
    return scaling
...
function QuestieMap.ProcessQueue()
    ...
    if processed and (not next(mapDrawQueue) and (not next(minimapDrawQueue))) then
        if HBDPins.RequestWorldMapUpdate then
            HBDPins.RequestWorldMapUpdate()
        elseif HBDPins.UpdateWorldMap then
            HBDPins.UpdateWorldMap()
        end

        local QuestieRoute = QuestieLoader:ImportModule("QuestieRoute")
        if QuestieRoute and QuestieRoute.MarkCoordsDirty then
...
function QuestieMap:DrawManualIcon(data, areaID, x, y, typ)
    ...
    QuestieFramePool:ApplyWorldMapShiftClickThroughToIcon(icon)
...
function QuestieMap:DrawWorldIcon(data, areaID, x, y, showFlag)
    ...
    QuestieFramePool:ApplyWorldMapShiftClickThroughToIcon(iconMap)
