            ...
            if fadeTickerDirection then
                ...
                if fadeTickerValue < 0.3 then
                    ...
                    if (Questie.db.char.isTrackerExpanded and Questie.db.profile.trackerBackdropEnabled and Questie.db.profile.trackerBackdropFader) then
                        local backdropR, backdropG, backdropB = GetBackdropRGB()
                        trackerBaseFrame:SetBackdropColor(backdropR, backdropG, backdropB, math.min(Questie.db.profile.trackerBackdropAlpha, fadeTickerValue * 3.3))
                        if Questie.db.profile.trackerBorderEnabled then
                            local borderR, borderG, borderB = GetBorderRGB()
                            local borderAlpha = Questie.db.profile.trackerBorderAlpha or Questie.db.profile.trackerBackdropAlpha
                            trackerBaseFrame:SetBackdropBorderColor(borderR, borderG, borderB, math.min(borderAlpha, fadeTickerValue * 3.3))
                        end
                    end

                    -- Un-fade the Questie Icon
                    if (Questie.db.char.isTrackerExpanded and not Questie.db.profile.trackerHeaderEnabled and trackerHeaderFrame.questieIcon) then
                        trackerHeaderFrame.questieIcon:SetAlpha(fadeTickerValue * 3.3)
                    end

                    -- Un-fade the re-sizer
                    if Questie.db.char.isTrackerExpanded and trackerBaseFrame.sizer then
                        local floorAlpha = Questie.db.profile.sizerHidden and 0 or SIZER_IDLE_ALPHA
                        trackerBaseFrame.sizer:SetAlpha(math.max(floorAlpha, fadeTickerValue * 3.3))
                    end
