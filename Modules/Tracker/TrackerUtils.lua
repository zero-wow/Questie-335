...
local function _GetQuestLogFrame()
    return QuestLogExFrame or ClassicQuestLog or QuestLogFrame
end

local function _GetQuestLogScrollBar()
    if QuestLogScrollFrameScrollBar then
        return QuestLogScrollFrameScrollBar
    end

    if QuestLogListScrollFrame and QuestLogListScrollFrame.ScrollBar then
        return QuestLogListScrollFrame.ScrollBar
    end

    return QuestLogListScrollFrameScrollBar
end

local function _GetQuestLogIndex(quest)
    ...
    -- Ascension custom quests can surface their ID as a string while the client returns a number.
    local numEntries = GetNumQuestLogEntries and select(1, GetNumQuestLogEntries()) or 0
    local questIdText = tostring(questId)
    local questName = quest.name or quest.Name
    local nameMatch
    for index = 1, numEntries do
        local title, _, _, isHeader, _, _, _, liveQuestId = GetQuestLogTitle(index)
        if not isHeader then
            if liveQuestId ~= nil and tostring(liveQuestId) == questIdText then
                return index
            end
            if questName and title == questName then
                nameMatch = nameMatch or index
            end
        end
    end
...
function TrackerUtils:ShowQuestLog(quest)
    local questLogIndex = _GetQuestLogIndex(quest)
    if not questLogIndex then
        Questie:Debug(Questie.DEBUG_DEVELOP, "[TrackerUtils:ShowQuestLog] Quest not found in active quest log: " .. tostring(quest and quest.Id))
        return false
    end

    local questFrame = _GetQuestLogFrame()
    if not questFrame then
        Questie:Debug(Questie.DEBUG_DEVELOP, "[TrackerUtils:ShowQuestLog] Quest log frame is unavailable")
        return false
    end
    ...
    -- QuestLogEx and Ascension's quest-log variants do not always expose the same scrollbar.
    local scrollBar = _GetQuestLogScrollBar()
    if scrollBar and scrollBar.GetValueStep and scrollBar.SetValue then
        local scrollStep = scrollBar:GetValueStep()
        if type(scrollStep) == "number" and scrollStep > 0 then
            scrollBar:SetValue(math.max(0, (questLogIndex - 3) * scrollStep))
        end
    end
...
function TrackerUtils:GetCompletionText(quest)
    ...
    local fallbackText = quest.Description and quest.Description[1] or quest.LocalizedName or quest.name
    if fallbackText then
        return fallbackText:gsub("%.", "")
    end
end
...
function TrackerUtils:FocusObjective(questId, objectiveIndex)
    ...
    for questLogQuestId in pairs(QuestiePlayer.currentQuestlog) do
        local quest = QuestiePlayer.currentQuestlog[questLogQuestId]
        if type(quest) ~= "table" then
            quest = QuestieDB.GetQuest(questLogQuestId)
        end
...
---@return string|nil
local function _GetQuestLogHeaderName(questId)
    if not questId then
        return
    end
    ...
    local currentHeader
    for questIndex = 1, questLogIndex do
        ...
        if isHeader then
            currentHeader = title
        elseif questIndex == questLogIndex then
...
local function _GetZoneName(zoneOrSort, questId)
    if zoneOrSort == nil then
        return _GetQuestLogHeaderName(questId) or "Unknown Zone"
    end
    local zoneName
    ...
    if sortObj == "byZone" or sortObj == "byZonePlayerProximity" or sortObj == "byZonePlayerProximityReversed" then
        ...
        elseif (zoneOrSort) < 0 then
            ...
            -- Some quests are only grouped under a quest-log header/category.
            zoneName = _GetQuestLogHeaderName(questId) or "Unknown Zone"
        end
    else
    ...
    for questId, quest in pairs(QuestiePlayer.currentQuestlog) do
        if type(quest) == "table" and quest.Id then
            ...
            if quest:IsComplete() == 1 or (not next(quest.Objectives)) then
                ...
                local count = 0
                for _, Objective in pairs(quest.Objectives) do
                    local needed = tonumber(Objective.Needed) or 0
                    local collected = tonumber(Objective.Collected) or 0

                    if needed > 0 then
                        percent = percent + math.min(collected / needed, 1)
                    elseif Objective.Completed == true then
                        percent = percent + 1
                    end
                    count = count + 1
                end
                percent = count > 0 and (percent / count) or 0
...
function TrackerUtils:GetVoiceOverOverlayUI()
    if (not self:IsVoiceOverLoaded()) or type(VoiceOver) ~= "table" then
        return nil
    end

    local overlay = VoiceOver.QuestOverlayUI
    if type(overlay) ~= "table" then
        return nil
    end

    overlay.questPlayButtons = overlay.questPlayButtons or {}
    overlay.displayedButtons = overlay.displayedButtons or {}

    return overlay
end

function TrackerUtils:ShowVoiceOverPlayButtons()
    if self:GetVoiceOverOverlayUI() then
...
function TrackerUtils:UpdateVoiceOverPlayButtons()
    local overlay = self:GetVoiceOverOverlayUI()
    if overlay and overlay.CreatePlayButton then
        if Questie_BaseFrame.isSizing == true or Questie_BaseFrame.isMoving == true then
        ...
        for i = 1, 75 do
            ...
            if not isHeader then
                if not overlay.questPlayButtons[questId] then
                    ...
                    if overlay.questPlayButtons[questId] then
                        table.insert(overlay.displayedButtons, overlay.questPlayButtons[questId])
                    end
                end
            end
        end
