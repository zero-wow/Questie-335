...
function MapIconTooltip:Show()
    if QuestieFramePool.mapShiftClickThrough then
        return
    end
    ...
    local function handleMapIcon(icon)
        ...
        if (not icon.miniMapIcon) and not (iconData.Type == "available" or iconData.Type == "complete") and self.data.Id == iconData.Id then -- Recolor hovered icons
            ...
            entry.icon = icon;
            if iconData.isSingleSpawnMono then
                if Questie.db.profile.questObjectiveColors and iconData.IconColor then
                    icon.texture:SetVertexColor(iconData.IconColor[1], iconData.IconColor[2], iconData.IconColor[3], 1)
                else
                    icon.texture:SetVertexColor(0.6, 1, 1, 1)
                end
            elseif Questie.db.profile.questObjectiveColors then
    ...
    Tooltip.hoveredIconData = (self.data and self.data.ObjectiveData) and self.data or nil
    Tooltip._Rebuild = function(self)
        ...
        if self.hoveredIconData and next(self.questOrder) then
            _MapIconTooltip:RenderPfQuestObjectiveTooltip(self, self.hoveredIconData, self.questOrder)
        else
...
local function _StripColorCodes(text)
    if not text then return "" end
    return text:gsub("|c%x%x%x%x%x%x%x%x", ""):gsub("|r", "")
end

local function _GetPfProgressRGB(collected, needed)
    local maxVal = tonumber(needed) or 0
    local minVal = tonumber(collected) or 0
    if maxVal <= 0 then maxVal = 1 end

    local perc = minVal / maxVal
...
function _MapIconTooltip:GetSpawnTooltipInfo(iconData)
    local name = iconData.Name or UNKNOWN
    local id = iconData.ObjectiveTargetId
    local spawnType = UNKNOWN
    local levelStr = UNKNOWN

    if iconData.Type == "object" then
        spawnType = "Object"
        if id and id ~= 0 then
            name = QuestieDB.QueryObjectSingle(id, "name") or name
        end
    elseif iconData.Type == "event" then
        spawnType = l10n("Event")
    else
        spawnType = "Unit"
        if id and id ~= 0 then
            name = QuestieDB.QueryNPCSingle(id, "name") or name
            local minLevel = QuestieDB.QueryNPCSingle(id, "minLevel")
            local maxLevel = QuestieDB.QueryNPCSingle(id, "maxLevel")
            local rank = QuestieDB.QueryNPCSingle(id, "rank")
            if minLevel and maxLevel then
                if minLevel == maxLevel then
                    levelStr = tostring(minLevel)
                else
...
function _MapIconTooltip:RenderPfQuestObjectiveTooltip(tooltip, iconData, questOrder)
    local spawnInfo = self:GetSpawnTooltipInfo(iconData)
    local header = spawnInfo.name

    if spawnInfo.id and spawnInfo.id ~= 0 and Questie.db.profile.enableTooltipsNPCID then
        header = header .. " |cffcccccc(" .. spawnInfo.id .. ")|r"
    end

    tooltip:SetText(header, 0.3, 1, 0.8)
    tooltip:AddDoubleLine(l10n("Level") .. ":", spawnInfo.levelStr, 0.8, 0.8, 0.8, 1, 1, 1)
    tooltip:AddDoubleLine(l10n("Type") .. ":", spawnInfo.spawnType, 0.8, 0.8, 0.8, 1, 1, 1)

    for questId, textList in pairs(questOrder) do
        local quest = QuestieDB.GetQuest(questId)
        if quest then
            tooltip:AddLine("|cff555555[|cffffcc00!|cff555555]|r " .. quest.name, 1, 1, 0)

            if questId == iconData.Id and iconData.ObjectiveData and iconData.ObjectiveData.Needed then
                local objective = iconData.ObjectiveData
                local label = iconData.Name or spawnInfo.name
                local collected = objective.Collected or 0
                local needed = objective.Needed
                local r, g, b = _GetPfProgressRGB(collected, needed)
                tooltip:AddLine("|cffaaaaaa- |r" .. label .. ": " .. collected .. "/" .. needed, r, g, b)
            else
                for _, textData in pairs(textList) do
                    for textLine, nameTable in pairs(textData) do
                        local plain = _StripColorCodes(textLine)
                        local spawnName = next(nameTable)
                        local collected, needed, rest = plain:match("^(%d+)/(%d+)%s*(.*)$")
                        local label = spawnName or spawnInfo.name

                        if collected and needed then
                            local r, g, b = _GetPfProgressRGB(tonumber(collected), tonumber(needed))
                            tooltip:AddLine("|cffaaaaaa- |r" .. label .. ": " .. collected .. "/" .. needed, r, g, b)
                        elseif rest and rest ~= "" then
                            tooltip:AddLine("|cffaaaaaa- |r" .. label .. ": " .. rest, 0.8, 0.8, 0.8)
                        else
                            tooltip:AddLine("|cffaaaaaa- |r" .. plain, 0.8, 0.8, 0.8)
                        end
                    end
                end
            end
