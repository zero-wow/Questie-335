...
function QuestieLib:GetRGBForObjective(objective, quest, complete)
    ...
    if trackerColor == "questProgress" and quest then
        local cur, max = QuestieLib:GetQuestProgressTotals(quest, complete)
        return complete == -1 and "|cffff0000" or QuestieLib:GetPfQuestProgressColorHex(cur, max)
    end
...
--- pfQuest-style red -> yellow -> green gradient (matches pfMap.tooltip:GetColor)
---@param collected number
---@param needed number
---@return string
function QuestieLib:GetPfQuestProgressColorHex(collected, needed)
    local max = tonumber(needed) or 0
    local min = tonumber(collected) or 0
...
--- Sum objective progress like pfQuest tracker (not per-objective average)
---@param quest Quest
---@param complete number
---@return number cur
---@return number max
---@return number percent
function QuestieLib:GetQuestProgressTotals(quest, complete)
    if complete == 1 or quest.isComplete == true then
        return 1, 1, 100
    end
    ...
    if quest.Objectives then
        for _, objective in pairs(quest.Objectives) do
            if type(objective.Needed) == "number" and type(objective.Collected) == "number" then
                max = max + objective.Needed
                cur = cur + objective.Collected
            else
                max = max + 1
            end
        end
    ...
    return cur, max, math.min(100, (cur / max) * 100)
end

--- pfQuest-style quest title suffix: " (33%)" with gradient-colored number
---@param quest Quest
---@param complete number
---@return string
function QuestieLib:FormatPfQuestTrackerPercentSuffix(quest, complete)
    local cur, max, percent = QuestieLib:GetQuestProgressTotals(quest, complete)
    local colorHex = complete == -1 and "|cffff0000" or QuestieLib:GetPfQuestProgressColorHex(cur, max)
    return string.format(" |cffaaaaaa(%s%d%%|cffaaaaaa)|r", colorHex, math.ceil(percent))
end

--- pfQuest-style objective dash prefix (column B)
---@return string
function QuestieLib:FormatPfQuestTrackerObjectivePrefix()
    return "|cffffffff-|r"
end

--- pfQuest-style objective body without dash: "[0/4] Ice Shard Cluster"
---@param objective table
---@return string hasPrefix boolean @ false when body should span column C without a dash in column B
function QuestieLib:FormatPfQuestTrackerObjectiveBody(objective, quest, complete)
    if objective.fulfilled ~= nil and (not objective.Collected) then
        objective.Collected = objective.fulfilled
        objective.Needed = objective.required
    end
    ...
    local collected = objective.Collected
    local needed = objective.Needed

    if type(collected) ~= "number" or type(needed) ~= "number" then
        return "|cFFEEEEEE" .. objDesc .. "|r", false
    end

    local progressColor
    if Questie.db.profile.trackerColorObjectives == "questProgress" and quest then
        local cur, max = QuestieLib:GetQuestProgressTotals(quest, complete)
        progressColor = complete == -1 and "|cffff0000" or QuestieLib:GetPfQuestProgressColorHex(cur, max)
    else
        progressColor = QuestieLib:GetPfQuestProgressColorHex(collected, needed)
    end
    return progressColor .. string.format("[%d/%d]", collected, needed) .. "|r |cffffffff" .. objDesc .. "|r", true
end

--- pfQuest-style objective line: "- [0/4] Ice Shard Cluster"
---@param objective table
---@return string
function QuestieLib:FormatPfQuestTrackerObjective(objective)
    local body, hasPrefix = QuestieLib:FormatPfQuestTrackerObjectiveBody(objective)
    if hasPrefix then
        return QuestieLib:FormatPfQuestTrackerObjectivePrefix() .. " " .. body
    end
    return body
end
...
function QuestieLib:GetColoredQuestName(questId, showLevel, showState, blizzLike)
    ...
    if (not name) and quest and type(quest) == "table" then
        name = quest.LocalizedName or quest.name or ("Quest " .. tostring(questId))
        level = quest.level or level or 0
    ...
    if showState then
        local isComplete = quest and quest._liveQuestFallback and quest:IsComplete() or QuestieDB.IsComplete(questId)
        local questState = quest and quest._liveQuestFallback and quest or QuestieDB.GetQuest(questId)
        ...
        elseif isComplete == 0 and questState and questState.isComplete == true then
            name = name .. " " .. Questie:Colorize("(" .. l10n("Complete") .. ")", "green")
        end
    end

    local isRepeatable = quest and quest._liveQuestFallback and false or QuestieDB.IsRepeatable(questId)
    local isEventQuest = quest and quest._liveQuestFallback and false or QuestieDB.IsActiveEventQuest(questId)
    local isPvPQuest = quest and quest._liveQuestFallback and false or QuestieDB.IsPvPQuest(questId)

    return QuestieLib:PrintDifficultyColor(level, name, isRepeatable, isEventQuest, isPvPQuest)
end
