...
QuestieInit.Stages[1] = function() -- run as a coroutine
    ...
    if (not Questie.db.char.townsfolk) or (dbCompiledCount ~= Questie.db.char.townsfolkVersion) or (Questie.db.char.townsfolkClass ~= Townsfolk.GetPlayerBaseClass()) then
...
QuestieInit.Stages[3] = function() -- run as a coroutine
    ...
    coYield()
    QuestieLoader:ImportModule("QuestieRoute"):Initialize()
    ...
    coYield()
    -- Clear any stuck SmoothReset leftover from a previous session/error
    QuestieQuest._isResetting = nil
    QuestieQuest._resetNeedsAvailables = nil
    QuestieQuest._resetAgain = nil

    AvailableQuests.CalculateAndDrawAll(function()
        -- Available-quest pass must never leave in-log notes stale (objectives / turn-ins)
